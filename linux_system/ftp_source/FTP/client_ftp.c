...
{
	if(argc < 2){
		printf("myftp username:passwd@ftp-server-ip:ftp-server-port\n");
		return -1;
	}
	...
	char port[32] = {0};
	 ...
	 //%[^@]@%[^ ] 
	sscanf(argv[1],"%[^:]:%[^@]@%[^:]:%[^ ]",username,passwd,ip,port);
	printf("username :%s\n",username);
	printf("passwd :%s\n",passwd);
	printf("ip :%s\n",ip);
	printf("port:%s\n",port);
	...
	//LOGIN first;
	char loginfo[64] = {0};
	sprintf(loginfo,"%s:%s",username,passwd);
	x = send(sfd_client,loginfo,strlen(loginfo),0);
